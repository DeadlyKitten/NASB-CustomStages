...
#region Build Stage AssetBundle

            Debug.Log("Building stage assetbundle...");
            var stageAssetBundleName = $"{exportData.stage.gameUniqueIdentifier}.assetbundle";
            var stageBuild = new AssetBundleBuild
            {
                assetNames = new string[] { stageScenePath },
                assetBundleName = stageAssetBundleName
            };

            BuildPipeline.BuildAssetBundles(
                exportPath,
                new AssetBundleBuild[] { stageBuild },
                BuildAssetBundleOptions.None,
                BuildTarget.StandaloneWindows
            );
            Debug.Log("<color=green>Successfully built stage assetbundle!</color>");

#endregion

#region Build Music AssetBundle

            var stageMusicPath = AssetDatabase.GetAssetPath(exportData.stageMusic);

            var musicAssetBundleName = $"{exportData.MusicID}.assetbundle";
            var musicBuild = new AssetBundleBuild
            {
                assetNames = new string[] { stageMusicPath },
                assetBundleName = musicAssetBundleName
            };

            BuildPipeline.BuildAssetBundles(
                exportPath,
                new AssetBundleBuild[] { musicBuild },
                BuildAssetBundleOptions.None,
                BuildTarget.StandaloneWindows
            );
            Debug.Log("<color=green>Successfully built stage assetbundle!</color>");
...
#region Create package.json
            ...
            var package = new PackageJSON
            {
                ...
                stageBundlePath = stageBuild.assetBundleName,
                musicBundlePath = musicBuild.assetBundleName,
                stagePath = stageScenePath,
                musicId = exportData.MusicID,
                musicPath = stageMusicPath,
            ...
            zip.CreateEntryFromFile(Path.Combine(exportPath, stageAssetBundleName), stageAssetBundleName);
            zip.CreateEntryFromFile(Path.Combine(exportPath, musicAssetBundleName), musicAssetBundleName);
